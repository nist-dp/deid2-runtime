...
if __name__ == '__main__':
    ...
    # set up dataloaders
    dataset = UserDataset(X, y, users_to_idxs)
    dataloader_train = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_variable_size)
    ...
    for i in range(args.num_epochs):
        loss = train_weighted(model, dataloader_train, optimizer, criterion, device, scheduler=scheduler)
        ...
        if loss < best_loss:
            ...
            _, outputs = evaluate(model, dataloader_test, criterion, device, reduction='mean')
